...
int disassembler(const char*, const char*, amd_comgr_language_t, const char*);

int main(int argc, char** argv) 
{
    if (argc < 5) 
    {
        printf("Usage : amd-disassembler <.hsaco> <isa_name> <language> <output_file>\n");
    ...
    amd_comgr_language_t language = (amd_comgr_language_t) atoi(argv[3]);
    disassembler(argv[1], argv[2], language, argv[4]);
                 ...
                 const char*          output_file)
{
    ...
    status = amd_comgr_action_info_set_language(action, AMD_COMGR_LANGUAGE_HIP);
    checkError(status, "set language");
